...
void	RadioInterface::set_fineCorrectorDisplay (int v) {
	correctorDisplay	-> display (Khz (coarseOffset) + v);
	fineOffset	= v;
}

//	a slot called by the ofdmprocessor
void	RadioInterface::set_coarseCorrectorDisplay (int v) {
	correctorDisplay	-> display (Khz (v) + fineOffset);
	coarseOffset	= v;
}
/**
  *	clearEnsemble
...
void	RadioInterface::addtoEnsemble (const QString &s) {
	...
	if (running) {
		...
		QString temp;
		temp = dabSettings -> value ("autostartservice", "538             "). toString ();
		// temp = dabSettings -> value ("autostartservice", "triple j        "). toString ();
...
void	RadioInterface::showLabel	(QString s) {
	if (running) {
	   dynamicLabel	-> setText (s); fprintf (stderr, "[metrics] LABEL=%s\n", s.toStdString().c_str()); 
